...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  Future<void> _signInWithEmailAndPassword() async {
    ...
    setState(() => _isLoading = true);
    try {
      await ref.read(signInEmailProvider).execute(
            _emailController.text.trim(),
            _passwordController.text,
          );
      if (!mounted) return;
  ...
  Future<void> _signInWithGoogle() async {
    setState(() => _isLoading = true);
    try {
      await ref.read(signInGoogleProvider).execute();
      if (!mounted) return;
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => const HomePage()),
        (_) => false,
  ...
  Future<void> _forgotPassword() async {
    ...
    try {
      await ref
          ...
          .execute(email, continueUrl: 'https://teahealth-6d55b.web.app/reset');
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_pretty(e))),
      );
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  ...
  String _pretty(Object e) => e is Exception ? e.toString().replaceFirst('Exception: ', '') : '$e';
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned(top: -60, right: -40, child: _DecorativeCircle(size: 180, opacity: 0.15)),
          Positioned(bottom: -50, left: -30, child: _DecorativeCircle(size: 220, opacity: 0.12)),
          SafeArea(
            child: LayoutBuilder(
              builder: (context, constraints) {
...
// Decorative soft circle + OR divider (same as yours)
class _DecorativeCircle extends StatelessWidget {
  const _DecorativeCircle({required this.size, required this.opacity});
